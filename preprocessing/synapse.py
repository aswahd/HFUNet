# Preprocessing code adapted from: https://github.com/SLDGroup/MERIT/blob/main/utils/preprocess_synapse_data.py
import os
...
for split in splits:
    if split == "train":
        ct_path = "/home/wahd/datasets/Abdomen/RawData/Training/img"  # set your path to your trainset directory
        seg_path = "/home/wahd/datasets/Abdomen/RawData/Training/label"
        save_path = "/home/wahd/datasets/Abdomen/Preprocessed/Train"
    else:
        ct_path = "/home/wahd/datasets/Abdomen/RawData/Testing/img"  # set your path to your testset directory
        seg_path = "/home/wahd/datasets/Abdomen/RawData/Testing/label"
        save_path = "/home/wahd/datasets/Abdomen/Preprocessed/Test"

    if os.path.exists(save_path) is False:
        os.mkdir(save_path)
        os.mkdir(os.path.join(save_path, "imgs"))
        os.mkdir(os.path.join(save_path, "gts"))
    ...
    for ct_file in os.listdir(ct_path):
        ...
        for s_idx in range(ct_array.shape[0]):
            ...
            # Ignore if the slice is empty
            if np.sum(seg_array_s > 0) < 10:
                continue

            slice_no = "{:03d}".format(s_idx)
            new_ct_name = (
                ct_number.replace("img", "case") + "_slice" + slice_no + ".npy"
            )
            np.save(os.path.join(save_path, "imgs", new_ct_name), ct_array_s)
            np.save(os.path.join(save_path, "gts", new_ct_name), seg_array_s)
