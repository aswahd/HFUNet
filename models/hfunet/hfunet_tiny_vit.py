...
@register_model("hfunet_tiny_vit")
class HFUNet(nn.Module):
    ...
    def __init__(self, config: DotDict):
        super().__init__()
        # Since this is a fixed model, the configs are known beforehand

        config.image_encoder = DotDict.from_dict(
            {
                "name": "tiny_vit_with_fpn_neck",
                # "embed_dims": [64, 128, 160, 320, 320, 256], # Patch embedding to neck
                "embed_dims": [256, 256, 256, 256, 256, 256],
                "strides": [2, 2, 1, 1, 1],
            }
        )
